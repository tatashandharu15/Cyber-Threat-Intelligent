// Command server runs the User service HTTP API.
package main
...
import (
	...
	"github.com/siberindo/cti/services/user-service/internal/api"
	"github.com/siberindo/cti/services/user-service/internal/config"
...
func main() {
	...
	// The issuer is used only to verify inbound tokens; the User service does not
	// mint them.
	...
	log.Info("user-service starting", slog.Int("port", cfg.HTTPPort), slog.String("env", cfg.Env))
