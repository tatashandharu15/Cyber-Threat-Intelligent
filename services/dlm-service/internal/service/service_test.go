...
func validInput() CreateFindingInput {
	return CreateFindingInput{
		FindingType: "credential_reference", Title: "Leaked creds on paste site",
...
func TestCreateFindingPublishes(t *testing.T) {
	...
	if len(pub.events) != 1 || pub.events[0].topic != types.TopicFindingCreatedDLM {
		t.Fatalf("expected one finding.created.dlm event, got %+v", pub.events)
	}
	ev, ok := pub.events[0].value.(types.FindingCreated)
	if !ok || ev.FindingID != f.ID || ev.SourceModule != types.ModuleDLM {
...
func TestCreateFindingRejectsLiveURL(t *testing.T) {
	svc, _, _ := newSvc()
	in := validInput()
	live := "https://evil.example.com/phish"
	in.ContentURL = &live
...
func TestCreateFindingAcceptsDefangedURL(t *testing.T) {
	svc, _, _ := newSvc()
	in := validInput()
	defanged := "hXXps://evil.example.com/phish"
	in.ContentURL = &defanged
...
func TestEscalatePublishes(t *testing.T) {
	...
	if len(pub.events) != 1 || pub.events[0].topic != types.TopicFindingEscalatedDLM {
		t.Fatalf("expected finding.escalated.dlm event, got %+v", pub.events)
	}
