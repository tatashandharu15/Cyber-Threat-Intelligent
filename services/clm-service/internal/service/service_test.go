...
func (f *fakeStore) AddAffectedUsers(_ context.Context, _, id string, emails []string) error {
	if fn, ok := f.findings[id]; ok {
		count := len(emails)
		fn.AffectedUserCount = &count
		fn.UserCorrelationState = "completed"
		return nil
	}
	return ErrNotFound
}
...
func (f *fakeStore) ListSources(_ context.Context, _ string) ([]domain.BreachSource, error) {
	return nil, nil
}
func (f *fakeStore) CreateSource(_ context.Context, c *domain.BreachSource) (*domain.BreachSource, error) {
	c.ID = "source-1"
	return c, nil
}
...
func validInput() CreateFindingInput {
	return CreateFindingInput{
		CredentialType: "hashed_credential", MaskedIndicator: "us****@ex*****.com",
		MaskingPolicyVersion: "v1", Severity: "high", ConfidenceScore: 0.9, DedupKey: "abc123",
	}
}
...
func TestCreateFindingPublishes(t *testing.T) {
	...
	if len(pub.events) != 1 || pub.events[0].topic != types.TopicFindingCreatedCLM {
		t.Fatalf("expected one finding.created.clm event, got %+v", pub.events)
	}
	ev, ok := pub.events[0].value.(types.FindingCreated)
	if !ok || ev.FindingID != f.ID || ev.SourceModule != types.ModuleCLM {
		t.Fatalf("unexpected event payload: %+v", pub.events[0].value)
	}
}

func TestCreateFindingBumpsCleartextSeverity(t *testing.T) {
	svc, _, _ := newSvc()
	in := validInput()
	in.CredentialType = "cleartext_credential"
	in.Severity = "low"
	...
	if f.Severity != "high" {
		t.Fatalf("expected cleartext_credential severity bumped to high, got %s", f.Severity)
	}
}

func TestCreateFindingKeepsCleartextCritical(t *testing.T) {
	svc, _, _ := newSvc()
	in := validInput()
	in.CredentialType = "cleartext_credential"
...
func TestCreateFindingValidation(t *testing.T) {
	...
	cases := []func(*CreateFindingInput){
		func(in *CreateFindingInput) { in.CredentialType = "bogus" },
		func(in *CreateFindingInput) { in.MaskedIndicator = "" },
		func(in *CreateFindingInput) { in.MaskingPolicyVersion = "" },
...
func TestEscalatePublishes(t *testing.T) {
	...
	if len(pub.events) != 1 || pub.events[0].topic != types.TopicFindingEscalatedCLM {
		t.Fatalf("expected finding.escalated.clm event, got %+v", pub.events)
	}
...
func TestCorrelateAffectedUsers(t *testing.T) {
	svc, st, _ := newSvc()
	f, _ := svc.CreateFinding(context.Background(), "tenant-1", validInput())
	if err := svc.CorrelateAffectedUsers(context.Background(), "tenant-1", f.ID, []string{"al****@ex*****.com", "bo****@ex*****.com"}); err != nil {
		t.Fatalf("correlate: %v", err)
	}
	got := st.findings[f.ID]
	if got.UserCorrelationState != "completed" || got.AffectedUserCount == nil || *got.AffectedUserCount != 2 {
		t.Fatalf("expected state=completed count=2, got state=%s count=%v", got.UserCorrelationState, got.AffectedUserCount)
	}
	if err := svc.CorrelateAffectedUsers(context.Background(), "tenant-1", f.ID, nil); !IsValidation(err) {
		t.Fatalf("expected validation error for empty emails, got %v", err)
	}
}
